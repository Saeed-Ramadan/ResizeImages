...
widthInput.addEventListener("keyup", () => {
    //getting height according to the ratio ckeckbox status
    const height = ratioInput.checked ?  widthInput.value / ogImageRatio : heightInput.value;
    heightInput.value = Math.floor(height);
});
heightInput.addEventListener("keyup", () => {
    //getting width according to the ratio ckeckbox status
...
const resizeAndDownload = () => {
    ...
    //1.0 is 100% quality where 0.7 is 70% of tatal. you can pass from 0.1 - 1.0
    ...
    a.click(); //clicking <a> element so ythe file download
}
